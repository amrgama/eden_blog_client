...
import FollowingButton from '../../account/userInfo/FollowingButton'
import FollowButton from '../../account/userInfo/FollowButton'


const AuthorInfo = ({author, authUser}) => {
    console.log("author", author, "authUser", authUser)
  return (
    <div>
        <span className='d-block' style={{width: "80px", height:"calc(80px - 1.5rem)"}}></span>
        <div className='user bg-white position-relative'>
            <div 
                className="p-2 bg-white rounded-circle border border-dark box-shadow-sm"
                style={{position: "absolute", left: "50%", transform: "translateX(-50%)", bottom: "calc(100% - 1.5rem)", zIndex: 1}}
            >
                {
                    !!!author?.vector &&
                    <span className="d-flex" style={{width: "70px", height:"70px"}}>
                        <BsPerson className='fs-1 m-auto' />
                    </span>
                }
